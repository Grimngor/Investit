...
The feature requires Google OAuth credentials in `.env`. Google login is shown only when OAuth credentials and the email allowlist are configured. Repeat Google logins use account selection rather than forced consent and do not fail when Google omits a repeat-login refresh token. The separate Gmail connection flow can still force consent to recover a refresh token when Gmail import is disconnected. For public users, the Google OAuth app may need verification because Gmail read-only access is a restricted scope.
